...
distance = zeros(((S[1]-S[0])//increment+1,T,algo_count))
...
for s in range(S[0],S[1]+1,2):
  for t in range(T):
      ...
      # noise_magnitude = 1/m ** 0.5 * 0.1
      noise_magnitude = 1/m * 0.1
      noise = random.randn(m)
      b = A.dot(x0) +  noise * noise_magnitude
      delta1 = 1 * norm(noise) * noise_magnitude
      # delta2 = delta1 ** 2 
      ...
      constraints_l1  = [cp.norm(A @ x_l1 - b) <= delta1 ]
      ...
      constraints_rwl1 = [cp.norm(A @ x_rwl1 - b,2) <= delta1 ]
      ...
      if norm(x_rwl1.value-x0) <= recovery_criterion:
          recovery[(s - S[0])//increment,t,0] = 1
      
      distance[(s - S[0])//increment,t,0] = norm(x_rwl1.value-x0)
      ...
      constraints_rwl1_prox = [cp.norm(A @ xp_rwl1_prox - A @ xn_rwl1_prox - b,2) <= delta1]
      ...
      if norm(xp_rwl1_prox.value-xn_rwl1_prox.value-x0) <= recovery_criterion:
          recovery[(s - S[0])//increment,t,1] = 1
      
      distance[(s - S[0])//increment,t,1] = norm(xp_rwl1_prox.value-xn_rwl1_prox.value-x0)
      ...
      constraints_rwl1_star = [cp.norm(A @ xp_rwl1_star - A @ xn_rwl1_star - b,2) <= delta1]
      ...
      if norm(xp_rwl1_star.value-xn_rwl1_star.value-x0) <= recovery_criterion:
          recovery[(s - S[0])//increment,t,2] = 1
      
      distance[(s - S[0])//increment,t,2] = norm(xp_rwl1_star.value-xn_rwl1_star.value-x0)
...
savez("rwl_noisy_comparison",recovery = recovery,wall = wall,iter_count = iter_count,xset = xset,distance = distance)